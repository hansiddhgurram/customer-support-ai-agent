...
const getBaseURL = () => {
    ...
    const rawBaseURL = customURL || import.meta.env.VITE_API_BASE_URL || "http://127.0.0.1:8000";
...
export const analyzeTicket = async (ticket) => {
    const response = await API.post("/analyze-ticket", ticket);
    return response.data;
};
...
export const fetchSemanticClusters = async () => {
    const response = await API.get("/semantic-clusters");
    return response.data;
};
